...
/**
 * Get the product that a YITH WAPO add-on sold individually belongs to.
 *
 * YITH WooCommerce Product Add-Ons sells add-ons individually by adding a hidden placeholder product to
 * the cart, and keeping the actual selection in the cart item data. That placeholder product is private
 * and has no image, so the product URL and the image of the product the customer configured should be
 * sent to Kustom instead.
 *
 * @param array $cart_item WooCommerce cart item.
 *
 * @return WC_Product|false The product the add-on belongs to, or false if the cart item is not a YITH WAPO add-on sold individually.
 */
function kco_get_yith_wapo_addon_parent_product( $cart_item ) {
...
/**
 * Get the name of a YITH WAPO add-on sold individually.
 *
 * The placeholder product that YITH WooCommerce Product Add-Ons adds to the cart for add-ons sold
 * individually is named after the plugin itself, and is therefore meaningless to the customer. YITH hides
 * that name in the cart, and renames the order item when the order is created, but Kustom must be told
 * what the add-on actually is. The add-on is described by YITH the same way it is described in the cart.
 *
 * @param array $cart_item WooCommerce cart item.
 *
 * @return string The name of the add-on, or an empty string if the cart item is not a YITH WAPO add-on sold individually, or if the name could not be determined.
 */
function kco_get_yith_wapo_addon_name( $cart_item ) {
	if ( ! isset( $cart_item['yith_wapo_individual_addons'], $cart_item['yith_wapo_options'] ) ) {
		return '';
	}
	...
	if ( class_exists( 'YITH_WAPO' ) && class_exists( 'YITH_WAPO_Cart' ) ) {
	...
	if ( ! empty( $names ) ) {
		...
		$parent_product = kco_get_yith_wapo_addon_parent_product( $cart_item );

		// Without a product to refer to, there is nothing better to say than what the cart item already says.
		if ( ! $parent_product ) {
			return '';
		}
